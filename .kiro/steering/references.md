...
For this repo, you almost never need them. Protocol facts belong in `aiolumagen` (the sibling `pylumagen` git repo), not here. The integration consumes the parsed `LumagenState` and the typed enums (`Aspect`, `Input`, `Memory`, `Colorspace`, `HdrStatus`, `InputStatus`, `SourceMode`) — it doesn't care how the wire format works.
